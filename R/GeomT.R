...
draw_panel <- function(data,           # Data from aes()
  ...
  # Decrease the size of the circles.
  data$size <- data$size/max(data$size)

  # Change the "scale" to adapt the plot in the graphic.
  coords <- coord$transform(data, panel_params)

  # Plot circles as earthquakes.
  grid::circleGrob(
...
#' GeomTimeline
#'
#' These functions creates the visuals. geom_timeline need not be used

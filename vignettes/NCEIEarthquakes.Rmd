...
The loading process could be done using the `earqke_clean_data`.
...
The table above is an example of the output of the `earqke_clean_data`. I have only converted and cleaned the columns of interest (`LATITUDE`, `LONGITUDE`, `DATE`, `COUNTRY`, `LOCATION`, `EQ_PRIMARY`, and `TOTAL_DEATHS`).

_ATTENTION:_ The `earqke_location_clean` works internally of the `earqke_clean_data`, as a step of data manipulation.
...
To enhance the visuals, I have created the `theme_earqke`, which removes the background, grids, etc..

```{r,message=FALSE,warning=FALSE,out.width="100%",fig.height=3.5}
# Plotting with tailored theme.
tl_asia + NCEIEarthquakes::theme_earqke() -> tl_asia_w_theme
tl_asia_w_theme
```
...
The `NCEIEarthquakes` package has two features to be used in OpenStreet maps, the first one is a straightforward way to plot the earthquakes, and a single content in a popup.

```{r,message=FALSE,warning=FALSE,out.width="100%",fig.height=3.5}
# Using the cleaned dataset of Asia. Plotting DATE inside of the popups.
df_asia %>% 
       earqke_map(annot_col = 'DATE')
```

In the example above, the `DATE` of each earthquake is plotted. If you are going to use numeric variables, I kindly ask you to use `as.character` to convert the information in character.
...
df_asia %>% 
       earqke_map(annot_col = 'as.character(TOTAL_DEATHS)')
```

## 3.5. Earthquakes Popup in OpenStreet maps {#openstreet_2}

This is a step further of the simple `earqke_map` function. The `earqke_create_label` combines many columns into one column to plot it inside of a popup of OpenStreet maps.

```{r,message=FALSE,warning=FALSE,out.width="100%",fig.height=3.5}
# Creating a new data.
df_america <- df_clean %>% dplyr::filter(COUNTRY %in% c('USA','MEXICO','CANADA'),
                        YEAR > 1990 & YEAR < 2019)
# Creating a complex texts using the earqke_create_label.
df_america %>% 
